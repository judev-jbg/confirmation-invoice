...
Utilizado para notificar eventos en la confirmación de facturas.
"""
import asyncio
import aiohttp
import aiosmtplib
...
class NotificationManager:
    ...
    def __init__(self):
        ...
        self.slack_config = {
            ...
            "channel": os.getenv("SLACK_CHANNEL", "#confirmation-invoice"),
            "username": os.getenv("SLACK_USERNAME", "ConfirmationInvoice-Bot")
        }
    ...
    async def send_email_notification(self, subject: str, message: str, error_details: Optional[Dict[str, Any]] = None, is_critical: bool = False):
        ...
        try:
            ...
            email_msg["To"] = ", ".join(self.email_config["notification_emails"])

            priority_prefix = "[ERROR CRÍTICO]" if is_critical else "[ADVERTENCIA]"
            email_msg["Subject"] = f"{priority_prefix} - Confirmación de Facturas: {subject}"

            html_content = self._create_html_email_content(subject, message, error_details, is_critical)
            plain_content = self._create_plain_email_content(subject, message, error_details, is_critical)
            ...
            logger.info(f"Email notification sent successfully to {len(self.email_config['notification_emails'])} recipients")
            return True
    ...
    async def send_slack_notification(self, message: str, error_details: Optional[Dict[str, Any]] = None, is_critical: bool = False, type: str = "info"):
        ...
        try:
            slack_payload = self._create_slack_payload(message, error_details, is_critical, type=type)
    ...
    async def send_critical_notification(self, title: str, message: str, error_details: Optional[Dict[str, Any]] = None):
        ...
        email_task = self.send_email_notification(title, message, error_details, is_critical=True)
        slack_task = self.send_slack_notification(f"{title}: {message}", error_details, is_critical=True, type="error")
        ...
        if success:
            logger.info("At least one critical notification was sent successfully")
        else:
    ...
    def _create_html_email_content(self, subject: str, message: str, error_details: Optional[Dict[str, Any]], is_critical: bool) -> str:
        ...
        <body>
            <div class="header">
                <h2>{'🚨 Error' if is_critical else '⚠️ Advertencia'} - Confirmación de Facturas</h2>
            </div>
            <div class="content">
            ...
            <div class="footer">
                Confirmación de Facturas - Sistema Automatizado - Generado automáticamente
            </div>
        </body>
        </html>
        """
    ...
    def _create_plain_email_content(self, subject: str, message: str, error_details: Optional[Dict[str, Any]], is_critical: bool) -> str:
        ...
        content = f"""
{priority} - Confirmación de Facturas
    ...
    def _create_slack_payload(self, message: str, error_details: Optional[Dict[str, Any]], is_critical: bool, type: str = "info") -> Dict[str, Any]:
        ...
        blocks = [
            {
                ...
                "text": {
                    "type": "plain_text",
                    "text": f"{emoji} {status_text} - Confirmación de Facturas",
                    "emoji": True
                }
            },
            {
        ...
        blocks.append({
            ...
            "elements": [
                {
                    "type": "mrkdwn",
                    "text": f"Confirmación de Facturas - Sistema Automatizado | `Timestamp: {timestamp}`"
                }
            ]
        })
